/*
 * This file is part of DLect. DLect is a suite of code that facilitates the downloading of lecture recordings.
 *
 * Copyright © 2014 Lee Symes.
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <http://www.gnu.org/licenses/>.
 */
package org.dlect.ejb.internal.provder.impl.blackboard.au.uq;

import java.util.concurrent.ExecutionException;
import org.dlect.ejb.internal.provder.impl.blackboard.au.uq.rota.builders.UQRotaStorage;
import org.dlect.except.DLectException;
import org.dlect.except.DLectExceptionBuilder;
import org.dlect.internal.data.merge.PartialSemester;
import org.dlect.object.ResultType;
import org.dlect.plugin.provider.australia.uq.rota.UQRotaSemester;

/**
 *
 * @author lee
 */
public class UQSemesterBuilder {

    public static PartialSemester getSemesterFor(UQRotaStorage storage, int semId) throws DLectException {
        try {
            UQRotaSemester semester = storage.getSemester(semId);

            PartialSemester ps = new PartialSemester();
            ps.setSemesterCode(Integer.toString(semId));
            ps.setName(semester.getName());
            ps.setStartDate(semester.getStartDate());
            ps.setEndDate(semester.getEndDate());
            return ps;
        } catch (ExecutionException ex) {
            throw DLectExceptionBuilder.rethrow("Get Semester for failed", ResultType.UNIVERSITY_ERROR, ex);
        }
    }

    private UQSemesterBuilder() {
    }

}
